...
export type Slide = {
  ...
  avatar?: string;  // profile レイアウト用フェイス画像URL
  tags?: { label: string; icon?: string }[];  // profile レイアウト用タグ（興味など）
};
...
export const slides: Slide[] = [
  {
    ...
    image: "/logo.png",  // 省略可。指定すると左側に画像が表示される
  },
  {
    layout: "profile",
    title: "発表者名",
    avatar: "https://api.dicebear.com/7.x/personas/svg?seed=name&backgroundColor=c0aede",
    body: ["所属事業部", "入社年月"],
    tags: [
      { label: "Claude", icon: "https://upload.wikimedia.org/wikipedia/commons/8/8a/Claude_AI_logo.svg" },
      { label: "その他の興味" },
    ],
  },
  {
