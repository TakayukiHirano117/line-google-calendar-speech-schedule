...
\u30A4\u30D9\u30F3\u30C8\u60C5\u5831\u3092\u62BD\u51FA\u3067\u304D\u307E\u305B\u3093\u3067\u3057\u305F\u3002\u65E5\u4ED8\u3001\u6642\u523B\u3001\u5185\u5BB9\u3092\u542B\u3081\u3066\u8A71\u3057\u3066\u304F\u3060\u3055\u3044\u3002`,
  ...
  var CreateEventFromVoiceUseCase = class {
    ...
    execute(replyToken, messageId, userId) {
      ...
      if (result.success && result.eventId) {
        ...
        const flexMessage = this.flexMessageFactory.buildEventCreatedMessage(calendarEventData, eventUrl);
        ...
        this.lineMessaging.sendTextReply(replyToken, MESSAGE.CALENDAR_CREATION_FAILED);
      }
    }
  };
  ...
  var ShowHelpUseCase = class {
     ...
     * @param flexMessageFactory Flexメッセージファクトリー
     */
    constructor(lineMessaging, flexMessageFactory) {
      this.lineMessaging = lineMessaging;
      this.flexMessageFactory = flexMessageFactory;
    }
    /**
  ...
  var ShowLogoutUseCase = class {
     ...
     * @param lineMessaging LINE Messaging
     */
    constructor(oauth2ClientId, oauth2ClientSecret, lineMessaging) {
      this.oauth2ClientId = oauth2ClientId;
      this.oauth2ClientSecret = oauth2ClientSecret;
      this.lineMessaging = lineMessaging;
    }
  ...
  var SendAuthRequiredMessageUseCase = class {
     ...
     * @param flexMessageFactory Flexメッセージファクトリー
     */
    constructor(oauth2ClientId, oauth2ClientSecret, lineMessaging, flexMessageFactory) {
      this.oauth2ClientId = oauth2ClientId;
      this.oauth2ClientSecret = oauth2ClientSecret;
      this.lineMessaging = lineMessaging;
      this.flexMessageFactory = flexMessageFactory;
  ...
  // src/infra/line/LineMessaging.ts
  var LineMessaging = class {
    /**
     ...
     */
    sendTextReply(replyToken, messageText) {
      const requestBody = {
        replyToken,
        messages: [
          {
     ...
     */
    sendFlexReply(replyToken, flexContent) {
      const requestBody = {
        replyToken,
        messages: [
          {
  ...
  var FlexMessageFactory = class {
     ...
     * @returns Flexメッセージ
     */
    buildEventCreatedMessage(eventData, eventUrl) {
     ...
     * @returns Flexメッセージ
     */
    buildTodayEventsMessage(events) {
     ...
     * @returns Flexメッセージ
     */
    buildWeekEventsMessage(eventsByDate) {
      ...
      const dayContents = dateKeys.map((dateKey) => {
        ...
        const isToday = this.isSameDay(date, /* @__PURE__ */ new Date());
     ...
     * @returns Flexメッセージ
     */
    buildAuthRequiredMessage(authUrl) {
      return {
     ...
     * @returns Flexメッセージ
     */
    buildReauthRequiredMessage(authUrl) {
      return {
        altText: "\u518D\u8A8D\u8A3C\u304C\u5FC5\u8981\u3067\u3059",
        contents: {
          type: "bubble",
     ...
     * @returns Flexメッセージ
     */
    buildNoEventsMessage(title, subtitle, message) {
      return {
        altText: message,
        contents: {
          type: "bubble",
     ...
     * @returns イベント行コンテンツ
     */
    buildEventRowContent(event) {
      const timeText = event.isAllDay ? "\u7D42\u65E5" : this.formatTimeForFlex(event.startTime);
      return {
        type: "box",
        layout: "horizontal",
        contents: [
     ...
     * @returns 日付行コンテンツ
     */
    buildDayRowContent(date, events, isToday) {
      const dayNames = ["\u65E5", "\u6708", "\u706B", "\u6C34", "\u6728", "\u91D1", "\u571F"];
  ...
  // src/infra/google/UserCalendar.ts
  var CALENDAR_API_BASE = CONFIG.CALENDAR_API.BASE_URL;
  var UserCalendar = class {
    /**
     * @param userId LINEユーザーID
     * @param oauth2Manager OAuth2認証マネージャー
     */
    constructor(userId, oauth2Manager) {
      this.userId = userId;
  ...
  // src/infra/google/GoogleServiceAccountAuth.ts
  var GoogleServiceAccountAuth = class {
     ...
     * @returns アクセストークン
     */
    exchangeJwtForAccessToken(jwtToken) {
  ...
  // src/infra/google/SpeechToText.ts
  var SpeechToText = class {
     ...
     * @returns リクエストボディ
     */
    buildRequestBody(audioContentBase64) {
      return {
        config: {
          autoDecodingConfig: {},
     ...
     * @returns 認識されたテキスト
     */
    extractTranscript(responseData) {
     ...
     * @returns リクエストオプション
     */
    buildAuthorizedPostRequest(accessToken, requestBody) {
      return {
        method: "post",
        headers: {
          "Authorization": `Bearer ${accessToken}`,
  ...
  // src/infra/google/GeminiEventExtractor.ts
  var GeminiEventExtractor = class {
    /**
     ...
     */
    extractCalendarEventFromText(transcribedText) {
     ...
     * @returns プロンプト
     */
    buildEventExtractionPrompt(transcribedText) {
...
JSON\u5F62\u5F0F\u306E\u307F\u3092\u8FD4\u3057\u3001\u4ED6\u306E\u8AAC\u660E\u306F\u4E00\u5207\u4E0D\u8981\u3067\u3059\u3002`;
     ...
     * @returns イベントデータ
     */
    parseResponseToEventData(responseData) {
      var _a, _b, _c, _d, _e;
      try {
  ...
  var LineWebHookHandler = class _LineWebHookHandler {
    ...
    static processLineEvent(lineEvent) {
      ...
      const userId = ((_a = lineEvent.source) == null ? void 0 : _a.userId) || "";
